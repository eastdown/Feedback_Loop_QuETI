...
set_property SLEW FAST [get_ports {dac_data_o[*]}]
set_property DRIVE 8 [get_ports {dac_data_o[*]}]
...
# The ADC input clock is regenerated by the clock wizard before it reaches the
# ADC IOB registers.  Match the official Red Pitaya constraint: do not time the
# external ADC clock directly against that PLL-generated internal clock.
set_false_path -from [get_clocks adc_clk_p_i] \
               -to   [get_clocks clk_125_clk_bd_clk_wiz_0_0]


# 기존 exp_p_io[2]로 쓰면 기존 Red Pitaya expansion bus랑 엮일 수 있으니까, 아예 ttl_en이라는 독립 포트로 잡는 게 깔끔함.

set_property PACKAGE_PIN G17 [get_ports {ttl_en_0}] 
set_property IOSTANDARD LVCMOS33 [get_ports {ttl_en_0}]

set_property PACKAGE_PIN H16 [get_ports {ttl_en_1}] 
set_property IOSTANDARD LVCMOS33 [get_ports {ttl_en_1}]
